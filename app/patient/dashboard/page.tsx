...
import { Dish, Patient } from "@/lib/mock-data";
    ...
    <div className="min-h-screen bg-background text-foreground font-sans">
      ...
      <main className="p-6 max-w-4xl mx-auto w-full space-y-10 pb-32">
        ...
        {patient.allergies.length > 0 && (
          <section
            className="border-4 border-destructive bg-destructive/5 p-6 space-y-3"
            aria-labelledby="alerts-title"
          >
            <h2
              id="alerts-title"
              className="text-destructive font-black uppercase text-sm flex items-center gap-2 tracking-widest"
            >
            ...
            <div className="flex flex-wrap gap-2">
              {patient.allergies.map((a) => (
                <Badge
                  key={a}
                  variant="destructive"
                  className="rounded-none px-3 py-1 font-black text-xs uppercase border-none"
                >
                  Sans {a}
                </Badge>
              ))}
              {patient.dietaryRestrictions.map((d) => (
                <Badge
                  key={d}
                  variant="outline"
                  className="rounded-none px-3 py-1 font-black text-xs uppercase border-destructive text-destructive bg-white"
                >
                  Régime {d}
                </Badge>
              ))}
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-black uppercase tracking-widest border-b-2 border-border pb-2">
          ...
          {categories.map((cat) => {
            ...
            return (
              <section
                key={cat}
                className="space-y-6"
                aria-labelledby={`title-${cat}`}
              >
                <h3
                  id={`title-${cat}`}
                  className="text-xl font-black uppercase border-l-8 border-primary pl-4 tracking-tighter"
                >
                ...
                >
                  {availableDishes.length > 0 ? (
                      ...
                      return (
                        ...
                        >
                          <div className="space-y-1 flex-1">
                            <p
                              className="text-xl font-black uppercase group-hover:text-primary transition-colors"
                              aria-hidden="true"
                            >
                              {dish.name}
                            </p>
                    ...
                    <div className="p-8 border-2 border-dashed border-border bg-muted/20 text-center space-y-2">
                      <p className="font-black uppercase text-sm opacity-40">
                        Aucune option compatible disponible
                      </p>
                      <p className="text-xs font-bold text-muted-foreground italic">
                        Contactez le service diététique
                      </p>
                    </div>
                  )}
                </div>
